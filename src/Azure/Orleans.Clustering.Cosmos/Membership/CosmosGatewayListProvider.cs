...
internal partial class CosmosGatewayListProvider : IGatewayListProvider
{
    ...
    public async Task InitializeGatewayListProvider()
    {
        ...
        {
            LogErrorInitializingProvider(ex);
            throw;
        }
    }
    ...
    private partial void LogErrorInitializingProvider(Exception ex);

    [LoggerMessage(
        Level = LogLevel.Error,
        Message = "Error reading gateway list from Azure Cosmos DB"
    )]
    private partial void LogErrorReadingGatewayList(Exception ex);
}